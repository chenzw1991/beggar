layui.use(['element', 'form', 'layer', 'upload'], function () {
    ...
    var paramSkip = function () {
        ...
        $('.timo-search-box [name]').each(function (key, val) {	
            ...
            } else if ($(val).attr("required")){
            	var msg = $(val).attr("placeholder");
            	error = true;
            	alert(msg ? msg : "缺少必填项");
            }
        });
        if (error)
        	return false;
        try{
        	if(typeof(eval(queryFormCheck))=="function"){
        		error = queryFormCheck();//再需要补充验证的地方加上这个方法，截断提交则返回true
        		if (error)
                	return false;
        	}
        }catch(e){
        	console.log("no queryFormCheck");
        }
